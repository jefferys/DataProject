...
You can install *`DataProject`* from [GitHub](https://github.com/jefferys/DataProject) in any of the normal ways. The newest easy way to install stuff is with the `pak` package. To check if you have the "pak" package and install it if you don`t:

```{r need-package-pak, eval=FALSE}
...
```{r install-DataProject, eval=FALSE}
# Last commit from the default branch
pak::pkg_install("jefferys/DataProject")
```

To install the latest development version of *`DataProject`* from GitHub, along with all development dependencies:
...
To install a specific (tagged) release of *`DataProject`* or the most recent from any branch, specify the desired `<tag_or_branch_name>` instead of this placeholder and install with one of:
